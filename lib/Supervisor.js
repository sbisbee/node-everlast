...
var applyLogTag = function(logLine, child) {
  return util.format(logLine, child.id + '@' + child.process.pid);
};

/**
 * @param {restartStrategies.*} restartStrategy Defaults to OneForOne if not
 * provided.
 * @param {Number} maxR The maximum number of restarts allowed in maxT seconds.
 * Defaults to 3.
 * @param {Number} maxT The time window in seconds to count restarts in.
 * Defaults to 1 seconds.
 * @param {Boolean} augmentLog Whether or not Supervisor should modify
 * stdout/err with the child id and PID. Defaults to false.
 */
var Supervisor = function(restartStrategy, maxR, maxT, augmentLog) {
  //list of child specs
  this.children = [];
...
Supervisor.prototype.startChild = function(spec) {
  ...
  self.children[idx].process.on('exit', function onExit(code, signal) {
    ...
    if(self.children[idx].state === CHILD_STATES.restarting) {
      err = self.startChild.call(self, idx);
      ...
      if(self.restartStrategy) {
        self.restartStrategy.process(idx);
      }
    }
  });
  self.children[idx].process.stdout.on('data', function(chunk) {
    process.stdout.write(
      (self.augmentLog) ? applyLogTag(chunk.toString(), self.children[idx]) : chunk);
  });
  self.children[idx].process.stderr.on('data', function(chunk) {
...
/**
 * Gets the child ref at the given index.
 *
 * @param {Number} idx
 * @returns {Object|Null} The ref if there is one, otherwise null.
 */
Supervisor.prototype.getChild = function(idx) {
  return (this.children[idx]) ? {
    idx: idx,
    id: this.children[idx].id,
    pid: this.children[idx].pid
  } : null;
};
